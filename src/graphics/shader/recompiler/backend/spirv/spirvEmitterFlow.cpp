...
void EmitBarrier(EmitterState& state) {
	...
	const auto execution_scope =
	    tessellation || state.program.stage == ShaderType::Compute ||
	            state.program.stage == ShaderType::Mesh
	        ? spv::ScopeWorkgroup
	        : spv::ScopeSubgroup;
	state.builder.AddFunction(spv::OpControlBarrier, ConstantU32(state, execution_scope),
