...
void StoreDispatcherPhiEdge(ValueEmitContext& ctx, const DispatcherFunctionState& dispatcher,
	...
	for (const auto& [destination, value]: writes) {
		ctx.state.builder.AddFunction(spv::OpStore, destination, value);
	}
...
uint32_t EmitBvhIntersectRay(ValueEmitContext& ctx, const IR::Inst& inst) {
	// BVH intersection has no native per-opcode emitter; reuse the dedicated raytracing
	// emitter, which defines the instruction result itself.
