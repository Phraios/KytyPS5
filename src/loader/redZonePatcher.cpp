...
RedZonePatchResult PatchGuestInstructions(u64 segment_addr, u64 segment_size,
	...
	for (size_t function_index = 0; function_index < starts.size(); ++function_index) {
		...
		if (function.uses_red_zone) {
			...
			// Host-trapping sites (SSE4a EXTRQ/INSERTQ, MOVNTSS/SD, MONITORX/MWAITX)
			// are always relocated when the pass runs: the OS writes the fault
			// frame below rsp, into the guest red zone. Memory sites are instead
			// gated by protect_memory and collected below.
			for (const auto& [address, decoded]: function.instructions) {
				if (!IsHostTrappingInstruction(decoded.instruction) ||
				    !decoded.red_zone_live.any() || rewrite_sites.contains(address)) {
					continue;
				}
