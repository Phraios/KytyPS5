...
static bool TryEmulateSse4a(Context& context) {
	...
	// Fran: also accept the 0x79 register forms (length/index from a register).
	...
	if (prefix == 0x66) {
		// EXTRQ immediate form operates on its single register operand (rm);
		// the 0x79 register form addresses (dest, field) as (reg, rm).
		if (!register_form) {
			dest_index = rm;
		}
	}
	...
	uint64_t dest[2] {};
	uint64_t source      = 0;
	uint64_t field_high = 0;
	std::memcpy(dest, dest_xmm, sizeof(dest));
	std::memcpy(&source, src_xmm, sizeof(source));
	std::memcpy(&field_high, reinterpret_cast<const uint8_t*>(src_xmm) + sizeof(source),
	            sizeof(field_high));
	if (register_form) {
		// The 0x79 forms take the field length and index from a register:
		// EXTRQ reads them from the low 64 bits, INSERTQ from the high 64 bits.
		if (prefix == 0x66) {
			length = static_cast<uint8_t>(source & 0x3fu);
			index  = static_cast<uint8_t>((source >> 8u) & 0x3fu);
		} else {
			length = static_cast<uint8_t>(field_high & 0x3fu);
			index  = static_cast<uint8_t>((field_high >> 8u) & 0x3fu);
		}
	}
	if (prefix == 0x66) {
